...
describe('Starting tests for iot-coldchain-app', function() {
  ...
  it('website title should be iot-coldchain-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('iot-coldchain-app');
    })
  });

  it('network-name should be iot-coldchain-network@0.1.13',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('iot-coldchain-network@0.1.13.bna');
    });
  });

  it('navbar-brand should be iot-coldchain-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('iot-coldchain-app');
    });
  });
